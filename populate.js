...
co(function * () {
    ...
    try {
        ...
        for (var chat in results) {
            ...
            // We will insert these in bulk since we don't have to worry about duplicate transcripts
            for (var tran in tranSet) {
                ...
                try {
                    transArr.push(yield transcripts.insert(tranSet[tran]));
                    process.stdout.write("Inserting transcript for " + results[chat].chatID + "\r");
                } catch (ex) {
                    console.dir(ex);
                }
            }
        ...
        console.log("\nInserted " + transArr.length + " transcript objects.");
